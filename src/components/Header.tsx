...
const Header: React.FC<HeaderProps> = ({
  ...
  const getDifficultyDesc = (d: Difficulty) => {
    switch (d) {
      case "EASY":
        return "Top 300 words. Simple speed.";
      case "MEDIUM":
        return "Sentences & Punctuation.";
      case "HARD":
        return "Numbers, Symbols & Complex words.";
    }
  };
    ...
    <header className="w-full max-w-4xl flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
      <div className="flex items-center gap-3">
        ...
        <div className="flex flex-col">
          <h1
            className="text-4xl md:text-5xl font-black italic tracking-tighter text-black leading-none"
              ...
              <div className="flex gap-2">
                {(["EASY", "MEDIUM", "HARD"] as Difficulty[]).map((d) => (
                  <button
                    ...
                    className={`text-xs font-bold px-2 py-1 rounded border-2 border-black ${
                      difficulty === d
                        ? "bg-black text-white"
