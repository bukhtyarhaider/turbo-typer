...
const TypingArea: React.FC<TypingAreaProps> = ({ text, input, onInputChange, status, onRestart, report }) => {
    ...
    return text.split('').map((char, index) => {
      ...
      if (index < input.length) {
        if (inputChar === char) {
          ...
          className += "text-red-500 bg-red-100 font-bold line-through decoration-4";
        }
      } else if (index === input.length) {
        className += "bg-yellow-300 border-b-4 border-black animate-pulse text-black";
      } else {
        ...
        <div className="w-full text-center z-10 animate-fade-in-up flex flex-col items-center">
             ...
             <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8 w-full max-w-2xl mx-auto">
                 ...
                 <div className="bg-white border-4 border-black p-4 rounded-xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col items-center justify-center relative overflow-hidden">
                    <div className="text-gray-500 font-bold text-xs uppercase mb-1 z-10">NET WPM</div>
                    <div className="text-5xl font-black text-blue-600 z-10">{report.finalWpm}</div>
                    <Trophy className="absolute -bottom-4 -right-4 text-blue-100 w-24 h-24 transform -rotate-12 z-0"/>
                 </div>

                 {/* RANK */}
                 <div className="bg-white border-4 border-black p-4 rounded-xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col items-center justify-center relative overflow-hidden">
                    <div className="text-gray-500 font-bold text-xs uppercase mb-1 z-10">RANK</div>
                    <div className={`text-5xl font-black z-10 ${
                        report.rank === 'S' ? 'text-yellow-500' :
                        report.rank === 'A' ? 'text-green-500' :
                        report.rank === 'B' ? 'text-blue-500' : 'text-gray-500'
                    }`}>{report.rank}</div>
                 ...
                 <div className="bg-white border-4 border-black p-4 rounded-xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col items-center justify-center">
                    <div className="text-gray-500 font-bold text-xs uppercase mb-1 flex items-center gap-1"><Star size={12}/> ACCURACY</div>
                    <div className={`text-4xl font-black ${report.accuracy > 90 ? 'text-green-500' : 'text-orange-500'}`}>{report.accuracy}%</div>
                 </div>

                 {/* Errors */}
                 <div className="bg-white border-4 border-black p-4 rounded-xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col items-center justify-center">
                    <div className="text-gray-500 font-bold text-xs uppercase mb-1 flex items-center gap-1"><AlertCircle size={12}/> ERRORS</div>
                    <div className="text-3xl font-black text-red-500">{report.errors}</div>
                 </div>
             </div>
             ...
             {report.history && report.history.length > 0 && (
                 <div className="w-full max-w-md bg-white border-4 border-black rounded-xl p-4 mb-6 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                     <h3 className="text-lg font-black uppercase border-b-2 border-gray-200 pb-2 mb-2 text-left">🏆 Top Records</h3>
                     <div className="flex flex-wrap gap-2">
                         {report.history.map((score, i) => (
                             <span key={i} className={`font-mono font-bold px-2 py-1 rounded border-2 border-black ${score === report.finalWpm ? 'bg-yellow-300' : 'bg-gray-100'}`}>
                                 #{i+1} {score} WPM
                             </span>
                         ))}
                     </div>
