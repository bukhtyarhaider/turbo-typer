...
const GameCanvas: React.FC<GameCanvasProps> = ({ 
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (status === GameStatus.FINISHED) {
          ...
          const coastMove = currentDrawSpeed > 0 ? (2 + currentDrawSpeed * 0.15) : 0;
      ...
      if (progress > 60 || status === GameStatus.FINISHED) {
          ...
          finishLineX = baseFinishX + distanceRemaining - finishCoastDistanceRef.current;
      ...
      powerUps.forEach(pu => {
        if (!pu.collected) {
           ...
           if (status === GameStatus.PLAYING && pu.x < carX + CAR_WIDTH - 20 && pu.x + 30 > carX + 20) {
      ...
      // --- CAR ---
      drawCar(ctx, carX, carY, currentDrawSpeed);
      ...
      if (currentDrawSpeed > 50) {
        ...
        for (let i = 0; i < 5; i++) {
          ...
          ctx.lineTo(width - (dashOffset % 300) - 50 + (currentDrawSpeed/2), ly);
          ctx.stroke();
        }
      }
  ...
  }, [speed, obstacles, powerUps, hasShield, onCollide, onPowerUpCollect, status, progress, difficulty]);

  // --- DRAWING HELPERS ---

  const drawCar = (ctx: CanvasRenderingContext2D, x: number, y: number, currentSpeed: number) => {
    // Drop Shadow
  ...
  const drawFinishLine = (ctx: CanvasRenderingContext2D, x: number, groundY: number, crossed: boolean) => {
  ...
  const drawShieldAura = (ctx: CanvasRenderingContext2D, x: number, y: number) => {
    const time = Date.now();
    ctx.save();
  ...
  const drawPowerUp = (ctx: CanvasRenderingContext2D, x: number, y: number, type: 'nitro' | 'shield') => {
  ...
  const drawClouds = (ctx: CanvasRenderingContext2D, width: number, height: number, offset: number) => {
    ...
    cloudPos.forEach(cloud => {
      ...
      ctx.arc(drawX + 40 * cloud.s, cy - 10 * cloud.s, 40 * cloud.s, 0, Math.PI * 2);
  ...
  const drawMountains = (ctx: CanvasRenderingContext2D, width: number, height: number, offset: number) => {
    ...
    for (let i = 0; i < count; i++) {
        ...
        const finalX = drawX < -mountainWidth ? drawX + width + mountainWidth : drawX;
