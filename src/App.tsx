...
const App: React.FC = () => {
  ...
  const { 
    obstacles, powerUps, hasShield, speed, 
    resetGameEngine, handleCollision, handlePowerUpCollect, 
    updateSpeed, updateEntities, spawnPowerUp, spawnObstacle 
  ...
  const handleFinish = useCallback((report: GameReport) => {
  ...
  const {
      targetText, inputText, stats, 
      resetTypingEngine, handleInputChange, updateStats, lastKeyTime
  } = useTypingEngine({ 
      status, 
      setStatus, 
  ...
  }, [status, speed, updateStats, updateSpeed, updateEntities, spawnPowerUp, lastKeyTime]);
  ...
  return (
    <div className="min-h-screen bg-sky-100 font-sans p-4 md:p-8 flex flex-col items-center">
      ...
      <footer className="mt-8 text-center text-gray-600 font-medium">
         <p>Type correctly to speed up. Watch the road incline increase on Hard mode!</p>
      </footer>
