...
export const useTypingEngine = ({
  ...
  const handleInputChange = useCallback(
    (newInput: string) => {
      ...
      if (newInput.length === targetText.length) {
        // Update stats one final time to show 0 remaining characters
        setStats((prev) => ({
          ...prev,
          progress: 100,
          remainingChars: 0,
  ...
  const calculateReport = (): GameReport => {
    ...
    // Gross WPM = total characters typed / 5 / minutes (uncorrected)
    ...
    const finalAccuracy =
      targetText.length > 0
        ? Math.round(
            ((targetText.length - stats.errors) / targetText.length) * 100
          )
        : 100;
    const adjustedAccuracy = Math.max(0, Math.min(100, finalAccuracy));
  ...
  const updateStats = useCallback(() => {
    ...
    // Average WPM = Gross WPM (characters typed / 5 / minutes)
    const durationInMinutes = (now - start) / 60000;
    const averageWpm =
      durationInMinutes > 0
        ? Math.round(inputText.length / 5 / durationInMinutes)
        : 0;

    // Instant WPM for speed calculation (keystrokes in last 1.2s)
    const WINDOW_MS = 1200;
    keyPressHistory.current = keyPressHistory.current.filter(
      (t) => now - t < WINDOW_MS
    );

    const validKeystrokesInWindow = keyPressHistory.current.length;
    const instantWpm = Math.round(
      (validKeystrokesInWindow / 5) * (60 / (WINDOW_MS / 1000))
    );

    // Calculate remaining characters
    const remainingChars = Math.max(0, targetText.length - inputText.length);
        ...
        inputText.length > 0
          ? Math.max(
              ...
              Math.min(
                100,
                Math.round(
                  ((inputText.length - prev.errors) / inputText.length) * 100
                )
              )
            )
          : 100,
